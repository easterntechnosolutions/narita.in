...
class ControllerCatalogInformation extends Controller {
	...
	protected function getForm() {
		...
		} elseif (!empty($data['information_description'])) {
			$data['breadcrumb_image'] = $data['information_description'][1]['breadcrumb_image'];
		} else {
			$data['breadcrumb_image'] = '';
		}
		
		$this->load->model('tool/image');

		if (isset($this->request->post['breadcrumb_image']) && is_file(DIR_IMAGE . $this->request->post['breadcrumb_image'])) {
			$data['imagethumb'] = $this->model_tool_image->resize($this->request->post['breadcrumb_image'], 100, 100);
		} elseif (!empty($information_info) && is_file(DIR_IMAGE . $data['information_description'][1]['breadcrumb_image'])) {
			$data['imagethumb'] = $this->model_tool_image->resize($data['information_description'][1]['breadcrumb_image'], 100, 100);
		} else {
