<?php
class Modeljacklbfranchiseenquiryblock extends Model {
	...
	public function checkRegisteredUser($data) {
	   ...
	   $query=$this->db->query("SELECT * FROM " . DB_PREFIX . "customer where email='".$data['subscribe_email']."', username='".$data['username']."'");
	...
	public function UpdateRegisterUsers($data,$status) {
	   ...
	   $query=$this->db->query("UPDATE  " . DB_PREFIX . "customer SET newsletter ='".$status."' where email='".$data['subscribe_email']."', username='".$data['username']."'");
