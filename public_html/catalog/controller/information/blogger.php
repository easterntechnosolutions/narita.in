...
class ControllerInformationBlogger extends Controller {
	...
	public function index() {
		...
		$language = $this->model_ishithemes_blogger->getLanguageByCode($this->session->data['language']);

		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_ishithemes_blogger->addComment($this->request->get['blogger_id'], $this->request->post, $language['language_id']);
		...
		$module = $this->model_ishithemes_blogger->getModule($blog_info['module_id']);
		...
		$data['allow_comments'] = ((isset($setting['comments']) && $setting['comments'] == 1)) ? '1' : '';
		$data['login_required'] = ((isset($setting['login']) && $setting['login'] == 1)) ? '1' : '';
		$data['auto_approve'] = ((isset($setting['auto_approve']) && $setting['auto_approve'] == 1)) ? '1' : '';
		...
		$blog_comments = $this->model_ishithemes_blogger->getBlogComments($this->request->get['blogger_id'], $language['language_id']);
	...
	public function blogs() {
		...
		$blogs = $this->model_ishithemes_blogger->getBlogs();

		foreach ($blogs as $blog) {
			$module_blog_data = array();

			$module = $this->model_ishithemes_blogger->getModule($blog['module_id']);

			$setting = json_decode($module['setting'],true);

			if (!empty($setting['status']) && $setting['status']) {
				$module_blogs = $this->model_ishithemes_blogger->getBlogsByModule($blog['module_id']);
			
			
				foreach ($module_blogs as $module_blog) {
				
				$total_comments = $this->model_ishithemes_blogger->getTotalBlogComments($module_blog['blogger_id']);
